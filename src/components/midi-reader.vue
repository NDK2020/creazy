<template>
  <div class="midi-reader flex-col-center w-full">
    <div class="flex-end gap-12">
      ...
      <a-switch
        v-if="game_id == '3'"
        v-model:checked="dr.include_track_relation"
        checked-children="include-track-relation"
    ...
    <a-textarea
      class="text-area"
      v-model:value="midi_info.output_str"
      placeholder="output"
      auto-size
    />
    ...
    <a-textarea
      class="text-area"
      v-model:value="midi_info.raw_str"
...
const get_dr_data = async () => {
  ...
  song_info.value = `
name: ${typeof file_path.value == "string" ? file_path.value.replace(/^.*[\\\/]/, "") : ""}
