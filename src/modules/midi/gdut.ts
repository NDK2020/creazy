...
export class GDUT {
  ...
  constructor(midi: MidiFile, include_track_relation = false) {
  ...
  get_output(
    ...
    let relation_inclucde_notes_number = Array(3)
      .fill(0)
      .map((_, index) => index);
    ...
    mc_cnt = 0;
    if (has_cutter) {
      let tmp = final_notes.filter((n, i) => i >= note_start && i <= note_end);
